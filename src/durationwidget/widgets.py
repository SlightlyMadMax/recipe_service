...
class LabeledNumberInput(Input):
    input_type = "number"
    template_name = 'custom_widgets/labeled_number_input.html'

    def __init__(self, label=None, attrs=None, type=None, max=None):
        ...
        if attrs is not None:
            ...
            self.max = attrs.pop('max', self.max)
...
class TimeDurationWidget(MultiWidget):
    ...
    template_name = "custom_widgets/duration_multiple_input.html"
    ...
    def __init__(self, attrs=None, show_days=True, show_hours=True, show_minutes=True, show_seconds=True):
        ...
        _widgets = []
        _widgets.append(LabeledNumberInput(label="Дни", type="days")) if show_days else None,  # Day
        _widgets.append(LabeledNumberInput(label="Часы", type="hours", max=24)) if show_hours else None,  # Hour
        _widgets.append(LabeledNumberInput(label="Минуты", type="minutes", max=59)) if show_minutes else None,  # Minute
        _widgets.append(LabeledNumberInput(label="Секунды", type="seconds")) if show_seconds else None,  # Seconds
